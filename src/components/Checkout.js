  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button variant="text" onClick={() => handleNewAddress(false)}>
          Cancel
        </Button>
      </Stack>
    </Box>
...
const Checkout = () => {
   ...
   * }
   */
  const deleteAddress = async (token, addressId, selected) => {
    try {
  ...
  const performCheckout = async (token, items, addresses) => {
    let walletBalance = localStorage.getItem("balance");
    walletBalance -= getTotalCartValue(items);
    let addressid = "";

    if (validateRequest(items, addresses)) {
      for (let prop in addresses.all) {
        if (addresses.all[prop]["address"] === addresses.selected) {
          addressid = addresses.all[prop]["_id"];
          break;
        }
      }
  ...
  useEffect(() => {
    ...
    onLoadHandler();
    getAddresses(window.localStorage.getItem("token"));
              ...
              {window.localStorage.getItem("LoggedInStatus") === "true" ? (
                <div>
                  {!addresses.all.length ? (
                    <Typography my="1rem">
                      No addresses found for this account. Please Add a new
                      Address.
                    </Typography>
                  ) : (
                    <div>
                      {addresses.all.map((item) => (
                        <div>
                          {item.address === addresses.selected ? (
                            <Box className="address-item selected">
                              <Typography
                                onClick={() =>
                              ...
                              <Button
                                ...
                                variant="text"
                                onClick={() =>
                                  deleteAddress(token, item["_id"], true)
                                }
                              >
                                DELETE
                              </Button>
                            ...
                            <Box className="address-item not-selected">
                              <Typography
                                onClick={() =>
                                  handleSelectAddress(item.address)
                                }
                              ...
                              <Button
                                ...
                                variant="text"
                                onClick={() =>
                                  deleteAddress(token, item["_id"], false)
                                }
                              >
                                DELETE
                              </Button>
                ...
                <Typography my="1rem">
                  No addresses found. Please Login to Add a Address.
                </Typography>
              )}
            </Box>
